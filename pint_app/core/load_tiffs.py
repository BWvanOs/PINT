...
def load_tiffs_raw(folderPath: str):
    ...
    for f in tiffFiles:
        with tifffile.TiffFile(f) as tif:
            # Stack each page into [C, Y, X] (no OME dim guessing)
            pages = [page.asarray() for page in tif.pages]
            imageArray = np.stack(pages, axis=0)

            # Try channel names from TIFF page tags first; then OME-XML; else dummy
        ...
        if not ch_names:
            ...
            # ensure length matches number of planes
            if len(ch_names) < nC:
